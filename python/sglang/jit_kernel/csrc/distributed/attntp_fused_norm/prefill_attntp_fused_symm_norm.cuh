...
struct FusedPrefillAttnTPSymmNormParams {
...
struct FusedPrefillAttnTPSymmNormTrait
...
SGL_DEVICE void symm_wait_for_group_epoch(
    const FusedPrefillAttnTPSymmNormParams& params, const uint32_t signal_offset, const uint32_t signal_epoch) {
...
symm_source_partial(const FusedPrefillAttnTPSymmNormParams& params, const uint32_t source) {
  if constexpr (Trait::kNumGPU == 2) {
...
template <typename Trait, bool kReduceOnly = false>
__global__ void fused_prefill_attntp_symm_norm_kernel(const FusedPrefillAttnTPSymmNormParams __grid_constant__ params) {
  ...
  if (actual_rows_signed < 0 || static_cast<uint32_t>(actual_rows_signed) > params.capacity || owner_start_signed < 0 ||
      owner_start_signed >= static_cast<int32_t>(Trait::kNumGPU)) {
...
#pragma unroll
  ...
  for (uint32_t work_row = blockIdx.x; work_row < work_rows; work_row += gridDim.x) {
    ...
    if constexpr (Trait::kOutputMode == sglang::jit_kernel::attntp::OutputMode::kSingleContributor) {
      if (params.rank != owner_start) continue;
      if (global_row >= actual_rows) continue;
    } else if constexpr (Trait::kOutputMode == sglang::jit_kernel::attntp::OutputMode::kTokenScattered) {
      const auto owner_range =
          sglang::jit_kernel::attntp::balanced_row_range(actual_rows, params.rank, Trait::kNumGPU, owner_start);
...
#pragma unroll
          for (uint32_t index = 0; index < kElementsPerVector; ++index) {
            output_values[index] = device::cast<DType>(activation_values[vector][index]);
          }
          const uint32_t storage_index = threadIdx.x * kVectorsPerThread + vector;
...
#pragma unroll
        for (uint32_t vector = 0; vector < kVectorsPerThread; ++vector) {
          const uint32_t storage_index = threadIdx.x * kVectorsPerThread + vector;
          reduced_values[vector].store(output_row, storage_index);
        }
      } else {
        const auto* residual_row =
            static_cast<const float*>(params.residual) + static_cast<uint64_t>(global_row) * kHiddenSize;
...
struct FusedPrefillAttnTPSymmNorm {
  using Trait = FusedPrefillAttnTPSymmNormTrait<
      DType,
      kHiddenSize,
      kOutputMode,
      kInternalPrecision,
      ...
      kBlockSize,
      kSignalBackoff>;
  static constexpr auto kernel = fused_prefill_attntp_symm_norm_kernel<Trait, false>;
  ...
  run(const tvm::ffi::Tensor partial,
    ...
    TensorMatcher({output_capacity_symbol, kHiddenSize})
        ...
        .with_device(device_symbol)
        .verify(residual_out);
    TensorMatcher({1}).with_dtype<int32_t>().with_device(device_symbol).verify(actual_rows).verify(owner_start);
    ...
    const int64_t output_capacity_i64 = output_capacity_symbol.unwrap();
    RuntimeCheck(capacity_i64 > 0 && capacity_i64 <= UINT32_MAX, "Symm capacity must fit uint32");
    ...
    if constexpr (Trait::kNumGPU == 2) {
      ...
      RuntimeCheck(
          multicast_signal_offset_bytes_i64 >= capacity_i64 * kHiddenSize * static_cast<int64_t>(sizeof(DType)),
    ...
    const uint32_t expected_output_capacity =
        Trait::kOutputMode == sglang::jit_kernel::attntp::OutputMode::kTokenScattered
            ? div_ceil(capacity, Trait::kNumGPU)
            : capacity;
    RuntimeCheck(
        output_capacity_i64 == expected_output_capacity,
        "Symm output capacity mismatch, expected ",
    ...
    FusedPrefillAttnTPSymmNormParams params{};
    if constexpr (Trait::kNumGPU == 2) {
    ...
    params.output_capacity = expected_output_capacity;
    ...
    RuntimeCheck(num_blocks > 0, "Symm kernel requires at least one CTA");
