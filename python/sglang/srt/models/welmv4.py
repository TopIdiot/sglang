...
def _welm_init_kv_mirror_last_q_indices(forward_batch: ForwardBatch) -> bool:
    ...
    cpu_last_q_indices = getattr(
        forward_batch, "welm_kv_mirror_last_q_indices_cpu", None
    )
    cpu_active_batch_indices = getattr(
        forward_batch, "welm_kv_mirror_active_batch_indices_cpu", None
    )
    if last_q_indices.numel() == 0 and active_batch_indices.numel() == 0:
        if cpu_last_q_indices is None:
            cpu_last_q_indices = []
        if cpu_active_batch_indices is None:
    ...
    if cpu_last_q_indices is None:
        ...
        if extend_seq_lens_cpu is not None:
            ...
            for seq_len in extend_seq_lens_cpu:
                cursor += int(seq_len)
                cpu_last_q_indices.append(cursor - 1)
        elif last_q_indices.device.type == "cpu":
            cpu_last_q_indices = [int(index) for index in last_q_indices.tolist()]
    if cpu_active_batch_indices is None and active_batch_indices.device.type == "cpu":
        cpu_active_batch_indices = [
            int(index) for index in active_batch_indices.tolist()
        ]
    if cpu_last_q_indices is not None:
        if cpu_active_batch_indices is None:
            cpu_active_batch_indices = list(range(len(cpu_last_q_indices)))
        if len(cpu_last_q_indices) != len(cpu_active_batch_indices):
            raise RuntimeError(
...
def _welm_update_contracted_dp_metadata(
    ...
    *,
    ...
    if (
        ...
        attn_tp_size = max(get_attention_tp_size(), 1)
        tokens_per_rank, remainder = divmod(new_local_num_tokens, attn_tp_size)
        ...
        forward_batch.num_token_non_padded_cpu = min(
            max(real_rows - local_start, 0), local_size
        )
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(
        ...
        if router_context is not None:
            if dump_this_layer:
                raise RuntimeError(
                    "WeLM tensor dump does not support owner-local routing"
                )
            ...
            if get_global_expert_distribution_recorder().recording:
                raise RuntimeError(
                    "expert distribution recording does not support owner-local routing"
                )
            router_hidden_states = router_context.local_rows(hidden_states)
            if hidden_states_fp32 is not None:
                router_hidden_states_fp32 = router_context.local_rows(
                    hidden_states_fp32
                )
        gather_expert_hidden = bool(
            router_context is not None
            and getattr(router_context, "gathers_hidden_states", False)
        )
        shared_output = None
        if not gather_expert_hidden and self.shared_expert is not None:
        ...
        if router_context is not None:
            hidden_states, topk_output = router_context.prepare_moe_inputs(
                hidden_states,
                topk_output,
            )
            num_tokens = hidden_states.shape[0]
        if gather_expert_hidden and self.shared_expert is not None:
            shared_output = self.shared_expert(hidden_states)
            if self.shared_expert_gate is not None:
                shared_output = (
                    F.sigmoid(self.shared_expert_gate(hidden_states)) * shared_output
    ...
    def validate_local_router(self) -> None:
        ...
        if backend not in (MoeRunnerBackend.TRITON, MoeRunnerBackend.DEEP_GEMM):
            ...
            raise NotImplementedError(
                "owner-local routing requires a MoE runner that does "
...
class Qwen2MoeDecoderLayer(nn.Module):
    def __init__(
        ...
        if self.token_owner_uses_global_tp_moe:
            self.mlp.validate_local_router()
        ...
        if (
            ...
            if enable_token_owner:
                ...
                residual_is_padded_slice = (
                    row_align > 1
                    and output_size is not None
                    and residual.shape[0] * row_align == output_size
                )
                if not residual_is_padded_slice:
