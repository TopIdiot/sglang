...
| `SGLANG_WELM_MTP_SAMPLE_DRAFT` | 未设置 | 默认与设为 `0` 时都使用 greedy draft，不捕获 sampling graph family。仅设为 `1` 时显式启用 draft sampling：verify greedy 则 draft greedy；verify 随机采样则 draft 继承请求的 temperature、top-p，候选集按 `SGLANG_WELM_MTP_DRAFT_SAMPLING_TOPK` 截断，并为可能的 sampling mode 分别捕获 graph family。tree proposal（`topk > 1`）不支持随机 draft sampling。 |
| `SGLANG_WELM_MTP_DRAFT_FIXED_TEMPERATURE` | 未设置 | 仅在 `SGLANG_WELM_MTP_SAMPLE_DRAFT=1` 时显式覆盖 draft temperature；未设置时继承 verify。值必须大于 `0`。 |
| `SGLANG_WELM_MTP_DRAFT_FIXED_TOP_P` | 未设置 | 仅在 `SGLANG_WELM_MTP_SAMPLE_DRAFT=1` 时显式覆盖 draft top-p；未设置时继承 verify。值必须在 `(0, 1]`。 |
| `SGLANG_WELM_MTP_DRAFT_SAMPLING_TOPK` | `8` | 仅在 `SGLANG_WELM_MTP_SAMPLE_DRAFT=1` 时生效，是 graph 与 eager 路径共用的固定候选 top-k。显式设置必须在 `(0, vocab size)` 内，越界直接报错；未设置时使用默认值 `8` 并打印 warning。draft 截断只影响 proposal 分布与 acceptance rate，最终输出分布仍由 target verify 保证。 |
